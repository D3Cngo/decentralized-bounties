    ...
    Multicall
{
    ...
    /// @dev The minimum amount of time left in an Sponsored after a new bid is created. Default: 15 minutes.
    ...
    /// @dev listingId => current winning bid in an Sponsored.
    ...
    /// @dev Lets a token owner list tokens for sale: Direct Listing or Sponsored.
    function createListing(ListingParameters memory _params) external override onlyListerRoleWhenRestricted {
        ...
        // Tokens listed for sale in an Sponsored are escrowed in Marketplace.
        if (newListing.listingType == ListingType.Sponsored) {
    ...
    function updateListing(
        ...
        bool isSponsored = targetListing.listingType == ListingType.Sponsored;

        // Can only edit Sponsored listing before it starts.
        if (isSponsored) {
            require(block.timestamp < targetListing.startTime, "Marketplace: Sponsored already started.");
        }
        ...
        if (targetListing.quantity != safeNewQuantity) {
            // If the new quantity for an Sponsored is `0`, cancel the Sponsored.
            if (isSponsored && safeNewQuantity == 0) {
                _cancelSponsored(targetListing);
                return;
            } else {
                ...
                // Escrow the new quantity of tokens to list in the Sponsored.
                if (isSponsored) {
    ...
    /// @dev Lets an account (1) make an offer to a direct listing, or (2) make a bid in an Sponsored.
    function offer(
        uint256 _listingId,
        ...
        // Both - (1) offers to direct listings, and (2) bids to Sponsoreds - share the same structure.
        ...
        if (targetListing.listingType == ListingType.Sponsored) {
            // A bid to an Sponsored must be made in the Sponsored's desired currency.
            newOffer.currency = targetListing.currency;
            // A bid must be made for all Sponsored items.
    ...
    /// @dev Lets an account close an Sponsored for either the (1) winning bidder, or (2) Sponsored creator.
    function closeSponsored(uint256 _listingId, address _closeFor) external override nonReentrant {
        Listing memory targetListing = listings[_listingId];

        require(targetListing.listingType == ListingType.Sponsored, "Marketplace: not an Sponsored.");

        Offer memory targetBid = winningBid[_listingId];

        // Cancel Sponsored if (1) Sponsored hasn't started, or (2) Sponsored doesn't have any bids.
        bool toCancel = targetListing.startTime > block.timestamp || targetBid.offeror == address(0);

        if (toCancel) {
            _cancelSponsored(targetListing);
        } else {
            require(targetListing.endTime < block.timestamp, "Marketplace: cannot close Sponsored before it has ended.");

            // No `else if` to let Sponsored close in 1 tx when targetListing.tokenOwner == targetBid.offeror.
            if (_closeFor == targetListing.tokenOwner) {
                _closeSponsoredForSponsoredCreator(targetListing, targetBid);
            }

            if (_closeFor == targetBid.offeror) {
                _closeSponsoredForBidder(targetListing, targetBid);
            }
        }
    }
    ...
    /// @dev Processes an incoming bid in an Sponsored.
    function handleBid(Listing memory _targetListing, Offer memory _incomingBid) internal {
        ...
        // Close Sponsored and execute sale if there's a buyout price and incoming offer amount is buyout price.
        if (
            _targetListing.buyoutPricePerToken > 0 &&
            incomingOfferAmount >= _targetListing.buyoutPricePerToken * _targetListing.quantity
        ) {
            _closeSponsoredForBidder(_targetListing, _incomingBid);
        } else {
    ...
    /// @dev Cancels an Sponsored.
    function _cancelSponsored(Listing memory _targetListing) internal {
        require(
        ...
        emit SponsoredClosed(_targetListing.listingId, _msgSender(), true, _targetListing.tokenOwner, address(0));
    }

    /// @dev Closes an Sponsored for an Sponsored creator; distributes winning bid amount to Sponsored creator.
    function _closeSponsoredForSponsoredCreator(Listing memory _targetListing, Offer memory _winningBid) internal {
    ...
    /// @dev Closes an Sponsored for the winning bidder; distributes Sponsored items to the winning bidder.
    function _closeSponsoredForBidder(Listing memory _targetListing, Offer memory _winningBid) internal {
    ...
    /// @dev Transfers tokens listed for sale in a direct or Sponsored listing.
    ...
    /// @dev Lets a module admin set Sponsored buffers
    function setSponsoredBuffers(uint256 _timeBuffer, uint256 _bidBufferBps) external onlyModuleAdmin {
        require(_bidBufferBps < MAX_BPS, "Marketplace: invalid BPS.");

        timeBuffer = uint64(_timeBuffer);
        bidBufferBps = uint64(_bidBufferBps);

        emit SponsoredBuffersUpdated(_timeBuffer, _bidBufferBps);
    }
