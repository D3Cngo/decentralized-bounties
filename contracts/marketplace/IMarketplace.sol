...
interface IMarketplace {
     ...
     *          `Direct`: Bounties listed for claim at a fixed price set by the DAO.
     *          `Sponsored`: Bounties listed for claim by anyone, regardless of DAO position or influence.
     */
    enum ListingType {
        Direct,
        Sponsored
    ...
    /**
     * @notice The information related to either (1) an offer on a direct listing, or (2) a bid in an Sponsored.
     *
     ...
     *      If the listing is of type `Sponsored`, the `Offer` is interpreted as a bid in an Sponsored.
     */
    struct Offer {
        uint256 listingId;
        address offeror;
     ...
     *                              'active' means NFTs can be bought from the listing. For Sponsoreds,
     *                              'active' means bids can be made in the Sponsored.
     *
     ...
     *                              For Sponsoreds: 'inactive' means bids can no longer be made in the Sponsored.
     *
     ...
     *                              to buy the NFT(s). For Sponsoreds: the currency in which the bidders must make bids.
     *
     * @param reservePricePerToken  For direct listings: this value is ignored. For Sponsoreds: the minimum bid amount of
     *                              the Sponsored is `reservePricePerToken * quantityToList`
     *
     * @param buyoutPricePerToken   For direct listings: interpreted as 'price per token' listed. For Sponsoreds: if
     *                              `buyoutPricePerToken` is greater than 0, and a bidder's bid is at least as great as
     *                              `buyoutPricePerToken * quantityToList`, the bidder wins the Sponsored, and the Sponsored
     *                              is closed.
     *
     * @param listingType           The type of listing to create - a direct listing or an Sponsored.
     */
    ...
    /**
     * @notice The information related to a listing; either (1) a direct listing, or (2) an Sponsored listing.
     *
    ...
    /// @dev Emitted when (1) a new offer is made to a direct listing, or (2) when a new bid is made in an Sponsored.
    event NewOffer(
        uint256 indexed listingId,
    ...
    /// @dev Emitted when an Sponsored is closed.
    event SponsoredClosed(
        uint256 indexed listingId,
        address indexed closer,
        bool indexed cancelled,
        address SponsoredCreator,
    ...
    /// @dev Emitted when Sponsored buffers are updated.
    event SponsoredBuffersUpdated(uint256 timeBuffer, uint256 bidBufferBps);

    /// @dev Emitted when the LISTER_ROLE restriction is updated.
    event ListingRestricted(bool restricted);

    /**
     * @notice Lets a token (ERC 721 or ERC 1155) owner list tokens for sale in a direct listing, or an Sponsored.
     ...
     * @dev NFTs to list for sale in an Sponsored are escrowed in Marketplace. For direct listings, the contract
     ...
     *         An Sponsored listing cannot be edited after the Sponsored has started.
     *
     ...
     *                              `_quantityToList` amount of NFTs to list for sale. For Sponsored listings, the contract
     *                               ensures that exactly `_quantityToList` amount of NFTs to list are escrowed.
     *
     * @param _reservePricePerToken For direct listings: this value is ignored. For Sponsoreds: the minimum bid amount of
     *                              the Sponsored is `reservePricePerToken * quantityToList`
     *
     * @param _buyoutPricePerToken  For direct listings: interpreted as 'price per token' listed. For Sponsoreds: if
     *                              `buyoutPricePerToken` is greater than 0, and a bidder's bid is at least as great as
     *                              `buyoutPricePerToken * quantityToList`, the bidder wins the Sponsored, and the Sponsored
     *                              is closed.
     *
     ...
     *                              'inactive' means NFTs cannot be bought from the listing. For Sponsoreds,
     *                              'inactive' means bids can no longer be made in the Sponsored.
     */
    ...
    /**
     * @notice Lets someone make an offer to a direct listing, or bid in an Sponsored.
     *
     ...
     *
     * @param _quantityWanted   For Sponsored listings: the 'quantity wanted' is the total amount of NFTs
     *                          being Sponsoreded, regardless of the value of `_quantityWanted` passed.
     *                          For direct listings: `_quantityWanted` is the quantity of NFTs from the
     *                          listing, for which the offer is being made.
     *
     * @param _currency         For Sponsored listings: the 'currency of the bid' is the currency accepted
     *                          by the Sponsored, regardless of the value of `_currency` passed. For direct listings:
    ...
    /**
     * @notice Lets any account close an Sponsored on behalf of either the (1) Sponsored's creator, or (2) winning bidder.
     *              For (1): The Sponsored creator is sent the the winning bid amount.
     *              For (2): The winning bidder is sent the Sponsoreded NFTs.
     *
     * @param _listingId The unique ID of the listing (the Sponsored to close).
     * @param _closeFor For whom the Sponsored is being closed - the Sponsored creator or winning bidder.
     */
    function closeSponsored(uint256 _listingId, address _closeFor) external;
}
